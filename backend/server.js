  ...
  async (req, res) => {
    ...
    try {
      ...
      const duplicateCheckQuery = `
        SELECT 
          (SELECT COUNT(*) FROM employees WHERE email = $1) as email_count,
          (SELECT COUNT(*) FROM employees WHERE phone_no = $2) as phone_count,
          (SELECT COUNT(*) FROM government_ids WHERE aadhar_no = $3) as aadhar_count,
          (SELECT COUNT(*) FROM government_ids WHERE pan_no = $4) as pan_count,
          (SELECT COUNT(*) FROM previous_employment WHERE pf_no = $5 AND pf_no IS NOT NULL) as pf_count,
          (SELECT COUNT(*) FROM previous_employment WHERE uan_no = $6 AND uan_no IS NOT NULL) as uan_count
      `;
      ...
      const duplicates = duplicateCheckResult.rows[0];
      const duplicateErrors = [];
      if (duplicates.email_count > 0) duplicateErrors.push('Email ID already exists');
      if (duplicates.phone_count > 0) duplicateErrors.push('Phone Number already exists');
      if (duplicates.aadhar_count > 0) duplicateErrors.push('Aadhar Number already exists');
      if (duplicates.pan_count > 0) duplicateErrors.push('PAN Number already exists');
      if (duplicates.pf_count > 0) duplicateErrors.push('PF Number already exists');
      if (duplicates.uan_count > 0) duplicateErrors.push('UAN Number already exists');
          ...
          for (let i = 0; i < employmentArray.length; i++) {
            ...
            await client.query(
              `INSERT INTO employment_history (
                employee_id, company_name, designation, last_project, start_date, end_date, doc_path, department
...
app.delete('/api/employees/delete-selected', async (req, res) => {
  const client = await pool.connect();
  ...
  try {
    ...
    const filePathsResult = await client.query(`
      SELECT 
        g.aadhar_file, g.pan_file, 
        s.signature_file, 
        e.doc_path AS education_doc, 
        eh.doc_path AS employment_doc
      FROM employees emp
      LEFT JOIN government_ids g ON emp.id = g.employee_id
      LEFT JOIN signatures s ON emp.id = s.employee_id
      LEFT JOIN education e ON emp.id = e.employee_id
      LEFT JOIN employment_history eh ON emp.id = eh.employee_id
      WHERE emp.id = ANY($1)
    `, [employeeIds]);

    // Collect all file paths
    const filePaths = [];
    filePathsResult.rows.forEach(row => {
      if (row.aadhar_file) filePaths.push(row.aadhar_file);
      if (row.pan_file) filePaths.push(row.pan_file);
      if (row.signature_file) filePaths.push(row.signature_file);
      if (row.education_doc) filePaths.push(row.education_doc);
      if (row.employment_doc) filePaths.push(row.employment_doc);
    });
    ...
    // Delete from tables with employee_id foreign key
    for (const table of relatedTables) {
      await client.query(`DELETE FROM ${table} WHERE employee_id = ANY($1)`, [employeeIds]);
    }

    // Delete from employees table using id
    await client.query(`DELETE FROM employees WHERE id = ANY($1)`, [employeeIds]);
    ...
    await client.query('ROLLBACK');
    console.error(err);
    res.status(500).json({ error: 'Error deleting selected records', details: err.message });
  } finally {
    client.release();
  }
});
